...
"""def download_model():
    #url = "https://your-storage-link.com/model.safetensors"
    url = "https://drive.google.com/uc?export=download&id=1W3OgC4NNtcsNdP0TDToDf2rPo5amG6AF"

    local_filename = "models_and_processor/model.safetensors"
    with requests.get(url, stream=True) as r:
        r.raise_for_status()
        with open(local_filename, 'wb') pip install pipx==0.1.0as f:
...
def predict_sentiment_v2(audio_path, processor_path, inverse_label_map, max_length=32000, device='cuda' if torch.cuda.is_available() else 'cpu'):
    """
    ...
    model = Wav2Vec2ForSequenceClassification.from_pretrained(model_files).to(device)
...
def main():
    ...
    if option == "Analyse exploratoire":
        ...
        if st.button("Prédire le sentiment"):
            sentiment = predict_sentiment_v2(audio_id, processor_path, inverse_label_map)
